...
## Purpose
TBD - created by archiving change add-usage-threshold-alert. Update Purpose after archive.
## Requirements
...
系统 SHALL 在 background 中定时获取启用告警租户的当日使用额度，并在超出阈值时触发额度告警。**轮询逻辑必须等待 store hydration 完成后再读取配置数据。**

#### Scenario: Store Hydration 等待
- **GIVEN** background service worker 启动或被唤醒
- **WHEN** alarm 触发 `pollDailyUsageAndAlert()` 执行
- **THEN** 系统首先等待 `tenantStore` 和 `settingStore` 的 `ready` 状态变为 `true`
- **AND** 等待超时时间不超过 5 秒
- **AND** 超时后记录警告日志并继续执行（使用当前状态）

#### Scenario: Hydration 完成后正确读取配置
- **GIVEN** store hydration 已完成（`ready === true`）
- **WHEN** `pollDailyUsageAndAlert()` 读取 `tenantStore.tenantList` 和 `settingStore.dailyUsageAlert`
- **THEN** 系统获取到完整的持久化数据
- **AND** 能正确识别启用告警的租户

#### Scenario: Background 定时检查触发告警
- **GIVEN** 某租户已启用当日额度告警且配置阈值 > 0
- **AND** store hydration 已完成
